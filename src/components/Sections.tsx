...
import { Camera, MapPin, Mail, Phone, ArrowRight, Heart } from 'lucide-react';
...
export function Awards() {
  ...
  return (
    <section id="awards" className="py-24 bg-[#0A0A0A] border-y border-white/5 overflow-hidden">
      ...
      <div className="relative w-full flex overflow-x-auto no-scrollbar snap-x snap-mandatory pb-8 px-6 md:px-12 gap-6">
        {awards.map((award, i) => (
          <motion.div 
            key={i}
            ...
            className="min-w-[280px] md:min-w-[350px] snap-center bg-askari-black border border-askari-gold/20 p-8 flex items-center justify-center text-center h-40"
          >
  ...
  return (
    <section className="py-32 relative overflow-hidden">
      <div className="absolute inset-0 z-0 opacity-20">
        <img src="https://picsum.photos/seed/bgfeatured/1920/1080" alt="Background" className="w-full h-full object-cover" />
      ...
      <div className="relative z-10 flex overflow-x-auto no-scrollbar snap-x snap-mandatory px-6 md:px-[10vw] gap-8 pb-12">
        {images.map((src, i) => (
          <motion.div 
            key={i}
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8, delay: i * 0.1 }}
            className="min-w-[300px] md:min-w-[400px] aspect-[3/4] snap-center relative group overflow-hidden"
          >
            <img src={src} alt="Waterberg Collection" className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105" />
            ...
            <div key={i} className={`flex flex-col md:flex-row items-center gap-12 ${i % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
              ...
              >
                <div className="aspect-[4/3] overflow-hidden">
                  <img src={block.img} alt="Behind the lens" className="w-full h-full object-cover" />
                </div>
              </motion.div>
              <motion.div 
  ...
  return (
    <section id="contact" className="relative py-32 bg-askari-black overflow-hidden">
      <div className="absolute inset-0 z-0 opacity-10">
        <img src="https://picsum.photos/seed/contactbg/1920/1080" alt="Background" className="w-full h-full object-cover" />
      </div>
        ...
        <div>
          ...
          <form className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input type="text" placeholder="Name" className="w-full bg-transparent border-b border-white/20 pb-3 focus:outline-none focus:border-askari-gold transition-colors" />
              <input type="email" placeholder="Email" className="w-full bg-transparent border-b border-white/20 pb-3 focus:outline-none focus:border-askari-gold transition-colors" />
            </div>
            <input type="tel" placeholder="Phone" className="w-full bg-transparent border-b border-white/20 pb-3 focus:outline-none focus:border-askari-gold transition-colors" />
            <select className="w-full bg-transparent border-b border-white/20 pb-3 focus:outline-none focus:border-askari-gold transition-colors text-white/70 appearance-none">
            ...
            <textarea placeholder="Message" rows={4} className="w-full bg-transparent border-b border-white/20 pb-3 focus:outline-none focus:border-askari-gold transition-colors resize-none"></textarea>
            <button type="button" className="bg-askari-gold text-askari-black px-8 py-4 uppercase tracking-widest text-sm font-medium hover:bg-askari-white transition-colors w-full md:w-auto">
              Send Message
            </button>
          </form>
        </div>
